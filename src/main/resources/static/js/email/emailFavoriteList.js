$(function () {
    ...
    var gridTable = $('#table').jqGrid({
        url: Util.getPath() + "/email/favorite/list",
        multiselect: true,
        colModel: [{
            ...
            formatter: function (value, options, row) {
                ...
                actions.push('<a href="javascript:void(0)" class="actions star-item" data-id="' + row.id + '" title="收藏"><i class="fa fa-star"></i></a>');
                return actions.join("");
            }
        }],
    ...
    $("#table").on("click", ".edit-item", function () {
        ...
        var id = $(this).attr("data-id");
        var data = gridTable.jqGrid('getRowData', id, true);
        $.post(Util.getPath() + "/email/" + data.emailType + "/star/" + id, {
            "star": '0'
        }, function (res) {
            if (res.code === 200) {
                Util.msgOk(res.data);
