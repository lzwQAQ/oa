$(function () {
    ...
    var gridTable = $('#table').jqGrid({
        url: Util.getPath() + "/emailsend/list",
        multiselect: true,
        colModel: [{
            ...
            formatter: function (value, options, row) {
                var actions = [];
                actions.push('<div class="email-state rr"></div> ');
    ...
    $("#table").on("click", ".edit-item", function () {
        ...
        $.post(Util.getPath() + "/emailsend/star/" + id, {
            "star": '1' === star ? '0' : '1'
        }, function (res) {
            if (res.code === 200) {
                Util.msgOk(res.data);
    ...
    function updateDeletes(ids) {
        Util.ask("确定移动到回收站？", function (index) {
            Util.closeLayer(index);
            Util.postLoading(Util.getPath() + "/emailsend/update/deletes", {ids: ids}, function (res) {
                if (res.code === 200) {
                    Util.msgOk("移动到回收站成功！");
                    gridTable.trigger("reloadGrid");
                } else {
