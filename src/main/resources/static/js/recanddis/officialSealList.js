$(function () {
    ...
    var gridTable = $('#table').jqGrid({
        url: Util.getPath() + "/officialseal/list",
        multiselect: true,
        colModel: [{
            label: '文件名称',
            name: 'fileName',
    ...
    $("#table").on("click", ".edit-item", function () {
        ...
        Util.innerIframe(Util.getPath() + "/officialseal/form/" + id);
    });
    ...
    $("#table").on("click", ".download-item", function () {
        var id = $(this).attr("data-id");
        window.open(Util.getPath() + "/officialseal/download/" + id, "_blank");
    });
    ...
    $("#btn-add").click(function () {
        Util.innerIframe(Util.getPath() + "/officialseal/form");
    });
    ...
    function deletes(ids) {
        Util.ask("确定删除？", function (index) {
            ...
            Util.postLoading(Util.getPath() + "/officialseal/deletes", {ids: ids}, function (res) {
                if (res.code === 200) {
                    Util.msgOk("删除成功！");
                    gridTable.trigger("reloadGrid");
                } else {
